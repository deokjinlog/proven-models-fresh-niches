...
NICHES = {
 ...
 "soho_closure":  dict(name="소상공인 폐업·정리", gloss="절차·재고처분·세정리",
    affords=["B8","B11","B3"], hunch=.8,
    kw=[r"going\s*out\s*of\s*business",
        r"(store|shop|restaurant|retail|business)\s*(closure|closing\s*down)",
        r"liquidat\w*\s*(inventory|stock|assets|sale)",
        r"wind(ing)?\s*down\s*(a|your|the|their)\s*(business|company|startup)",
        r"shut(ting)?\s*down\s*(a|your|the|their)\s*(business|company|store|shop)"]),
 "disability_access": dict(name="장애인 이동·접근성", gloss="이동·정보·또래",
    affords=["B7","B3","B9","B2"], hunch=.8,
    kw=[r"wheelchair", r"visually\s*impaired", r"(blind|low[- ]vision)\s*(people|users|community|and)",
        r"(disabilit(y|ies)|disabled)\s*(people|person|community|users?|access|employ|mobility)",
        r"assistive\s*(tech|device)\w*\s*(for|to)\s*(the\s*)?(disab|blind|deaf|impair)",
        r"accessib(le|ility)\s*(travel|transit|transport|building|venue|navigation|map)"]),
 "caregiver_support": dict(name="환자 보호자·간병 지원", gloss="정보·중개·소진",
    affords=["B3","B7","B4"], hunch=.75,
    kw=[r"family\s*caregiver", r"caregiver\s*(support|burnout|network|marketplace|matching|platform)",
        r"patient\s*advocate", r"care\s*coordination", r"caregiving[^.]{0,20}(family|support|app)"]),
 "sidejob_worker": dict(name="직장인 부업·사이드잡", gloss="수익화·발견·정산",
    affords=["B7","B9","B4"], hunch=.65,
    kw=[r"side\s*hustle", r"side\s*income", r"moonlight\w*", r"second\s*income",
        r"extra\s*income\s*(app|platform|source|stream)",
        r"earn\s*(extra|side)\s*(money|income|cash)"]),
 "small_landlord": dict(name="소규모 임대·건물 관리", gloss="임대료·수선·세입자",
    affords=["B8","B11","B2"], hunch=.6,
    kw=[r"(small|independent|diy)\s*landlord", r"landlord[^.]{0,20}(software|tool|app|platform|portal)",
        r"rental\s*property\s*(management|software|tool)", r"tenant\s*(management|screening|portal)",
        r"property\s*manage(ment|r)[^.]{0,20}(small|smb|independent|diy|self)"]),
 "craft_seller":  dict(name="수제·공예 창작자 판매", gloss="판매·발견·UGC",
    affords=["B1","B7","B9"], hunch=.6,
    kw=[r"handmade\s*(marketplace|goods|seller|shop|product)", r"artisan[^.]{0,20}(marketplace|sell|craft|goods)",
        r"\bcraft(er|ers|s)?\s*(marketplace|seller|sell|shop)", r"maker\s*marketplace",
        r"etsy\s*(for|alternative|competitor|like)", r"indie\s*(craft|maker)"]),
}
